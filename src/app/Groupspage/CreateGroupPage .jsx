...
const CreateGroupPage = (props) => {
  ...
  const fetchGroups = async () => {
    try {
      const response = await axios.get(
        `http://localhost:8080/groupRoute/groups?userId=${staticUserId}`,
        {
          headers: {
  ...
  const handleCreateGroup = async () => {
    try {
      await axios.post(
        `http://localhost:8080/groupRoute/group?userId=${staticUserId}`,
        {
          name: groupName,
          userId: staticUserId,
          description,
  ...
  const handleJoinGroup = async (groupId) => {
    try {
      const response = await axios.post(
        `http://localhost:8080/groupRoute/group/join/${groupId}`,
        { userId: staticUserId },
        {
          headers: {
            'Content-Type': 'application/json',
    ...
    <div className="min-h-screen bg-gray-100 p-10" style={{ marginTop: '3rem' }}>
      ...
      {groups.length > 0 && (
        <div className="mt-10">
          ...
          {groups.map((group) => {
  const isJoined = group.members?.includes(staticUserId);
